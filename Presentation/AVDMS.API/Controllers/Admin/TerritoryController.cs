...
namespace AVDMS.API.Controllers.Admin
{
    ...
    public class TerritoryController : CustomBaseController
    {
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> ImportRegion([FromQuery] ImportRequest request)
        {
            _response.IsSuccess = false;
            List<string[]> data = new List<string[]>();
            ExcelWorksheets currentSheet;
            ExcelWorksheet workSheet;
            List<ImportedRegion> lstImportedRegion = new List<ImportedRegion>();
            IEnumerable<RegionDataValidationErrors> lstRegionFailedToImport;
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                {
                    lstImportedRegion.Add(new ImportedRegion()
                    {
            ...
            if (lstImportedRegion.Count == 0)
            {
                _response.Message = "File does not contains any record(s)";
                return _response;
            }

            lstRegionFailedToImport = await _territoryRepository.ImportRegion(lstImportedRegion);

            _response.IsSuccess = true;
            _response.Message = "Region list imported successfully";

            #region Generate Excel file for Invalid Data
            if (lstRegionFailedToImport.ToList().Count > 0)
            {
                _response.Message = "Uploaded file contains invalid records, please check downloaded file for more details";
                _response.Data = GenerateInvalidRegionDataFile(lstRegionFailedToImport);
        ...
        private byte[] GenerateInvalidRegionDataFile(IEnumerable<RegionDataValidationErrors> lstRegionFailedToImport)
        {
            byte[] result;
            int recordIndex;
            ...
            using (MemoryStream msInvalidDataFile = new MemoryStream())
            {
                using (ExcelPackage excelInvalidData = new ExcelPackage())
                {
                    ...
                    foreach (RegionDataValidationErrors record in lstRegionFailedToImport)
                    {
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetTerritories_Region_State_City_List_ById(Territories_Region_State_City_Search parameters)
        {
            var vResultObj = await _territoryRepository.GetTerritories_Region_State_City_List_ById(parameters);
            _response.Data = vResultObj;
