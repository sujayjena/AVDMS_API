...
namespace AVDMS.API.Controllers.Admin
{
    ...
    public class AdminMasterController : CustomBaseController
    {
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> SaveFrequency(Frequency_Request parameters)
        {
            int result = await _adminMasterRepository.SaveFrequency(parameters);
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetFrequencyList(Frequency_Search parameters)
        {
            IEnumerable<Frequency_Response> lstRoles = await _adminMasterRepository.GetFrequencyList(parameters);
            _response.Data = lstRoles.ToList();
            _response.Total = parameters.Total;
            return _response;
        }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetFrequencyById(long Id)
        {
            if (Id <= 0)
            {
                _response.Message = "Id is required";
            ...
            {
                var vResultObj = await _adminMasterRepository.GetFrequencyById(Id);
                _response.Data = vResultObj;
            }
            return _response;
        }
